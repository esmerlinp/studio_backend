...
def change_user_password(user_id:int, new_password:int, sessionId=None) -> Optional[UserModel]:
...
def get_user_by_email(email) -> Optional[UserModel]:
    user = db.fetch_one(
        "SELECT * FROM usuarios WHERE scorreoelectronico = %s",
        (email,)
    )
